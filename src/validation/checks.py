...
def check_impossible_coordinates(events: pd.DataFrame) -> list[dict]:
    ...
    for col_x, col_y in [("x", "y"), ("end_x", "end_y")]:
        ...
        bad = events[
            (events[col_x].notna())
            & ((events[col_x] < -1) | (events[col_x] > PITCH_LENGTH + 1) |
               (events[col_y] < -1) | (events[col_y] > PITCH_WIDTH + 1))
        ]
        if len(bad):
            issues.append(_issue(
                "impossible_coordinates", "error", len(bad),
...
def check_missing_player_ids(events: pd.DataFrame) -> list[dict]:
    ...
    on_ball = events[events["type"].isin(
        ["pass", "shot", "carry", "dribble", "ball_recovery", "interception", "clearance", "duel"]
    )]
    if not len(on_ball):
    ...
    return [_issue("missing_player_ids", sev, missing,
...
def check_invalid_timestamps(events: pd.DataFrame) -> list[dict]:
    ...
    if bad_min:
        issues.append(_issue("invalid_timestamps", "error", bad_min, f"{bad_min} events with out-of-range minute"))
    if bad_sec:
        issues.append(_issue("invalid_timestamps", "error", bad_sec, f"{bad_sec} events with out-of-range second"))
    if not issues:
...
def check_team_name_consistency(events: pd.DataFrame, matches: pd.DataFrame) -> list[dict]:
    ...
    if unknown:
        return [_issue("team_name_consistency", "error", len(unknown),
...
def check_competition_match(events: pd.DataFrame, matches: pd.DataFrame) -> list[dict]:
    ...
    if missing:
        return [_issue("competition_mismatch", "error", len(missing),
...
def check_outliers(events: pd.DataFrame) -> list[dict]:
    ...
    if "shot_xg" in events.columns:
        ...
        if bad_xg:
            issues.append(_issue("outliers", "error", bad_xg, f"{bad_xg} shots with xG outside [0,1]"))
    if not issues:
...
def check_leakage(train: pd.DataFrame, test: pd.DataFrame) -> list[dict]:
    ...
    if overlap:
        return [_issue("leakage", "error", len(overlap),
