...
def zone_y(y: float) -> str:
    ...
    if y < HALF_SPACE_LEFT[0] or y > HALF_SPACE_RIGHT[1]:
        return "wide"
    if (
        HALF_SPACE_LEFT[0] <= y < HALF_SPACE_LEFT[1]
        or HALF_SPACE_RIGHT[0] <= y < HALF_SPACE_RIGHT[1]
    ):
        return "half_space"
    return "central"
...
def enrich_events(events: pd.DataFrame, matches: pd.DataFrame) -> pd.DataFrame:
    ...
    df["opp_goals_before"] = (
        df.groupby(["match_id", "opponent_team"])["_is_goal"].cumsum() - df["_is_goal"]
    )
    df["score_diff"] = (
    ...
    df["is_wide_band"] = df["y"].apply(
        lambda y: (not pd.isna(y)) and (y < TOUCHLINE_BAND or y > 80 - TOUCHLINE_BAND)
    )
