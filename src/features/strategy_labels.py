...
from src.config import ADJUSTMENTS
...
def _label_strategy_flags(possessions: pd.DataFrame, pass_agg: pd.DataFrame) -> pd.DataFrame:
    ...
    flags = pass_agg[
        [
            "match_id", "possession",
            "fb_inversion", "wide_final_third", "far_side_half_space",
            "third_man_combos", "risky_central_share", "central_buildup_share",
        ]
    ]
    df = possessions.merge(flags, on=["match_id", "possession"], how="left").fillna(
        {"fb_inversion": 0, "wide_final_third": 0, "far_side_half_space": 0,
         "third_man_combos": 0, "risky_central_share": 0.0, "central_buildup_share": 0.0}
    )
    ...
    df["s_aggressive_counterpress"] = (
        (df["counterpress_flags"] > 0) | ((df["recovery_type"].isin(["interception", "ball_recovery"])) & (df["start_x"] >= 50))
    ).astype(int)
...
def label_strategies(possessions: pd.DataFrame, events: pd.DataFrame) -> pd.DataFrame:
    ...
    passes = passes.sort_values(["match_id", "possession", "event_seconds", "timestamp", "index"] if "index" in passes.columns
    ...
    def agg_grp(grp: pd.DataFrame) -> dict:
        ...
        fb_zone = (grp["x"] >= 15) & (grp["x"] <= 55) & ((grp["y"] < TOUCHLINE_BAND) | (grp["y"] > 80 - TOUCHLINE_BAND))
        inversion = fb_zone & end_central & (grp["end_x"] > grp["x"])
        wide_ft = (grp["end_x"] > 50) & ((grp["end_y"] < TOUCHLINE_BAND) | (grp["end_y"] > 80 - TOUCHLINE_BAND))
    ...
    def in_half_space_local(y):
        return (HALF_SPACE_LEFT[0] <= y < HALF_SPACE_LEFT[1]) or (HALF_SPACE_RIGHT[0] <= y < HALF_SPACE_RIGHT[1])

    if passes.empty:
        agg = pd.DataFrame(
            {"match_id": [], "possession": [], "fb_inversion": [], "wide_final_third": [],
             "far_side_half_space": [], "third_man_combos": [], "risky_central_share": [],
             "central_buildup_share": []}
        )
    else:
        agg = passes.groupby(["match_id", "possession"], sort=False).apply(
            lambda g: pd.Series(agg_grp(g)), include_groups=False
        ).reset_index()
