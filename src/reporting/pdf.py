...
def _styles() -> dict:
    ...
    styles = {
        ...
        "h1": ParagraphStyle("H1", parent=ss["Heading1"], fontSize=15, textColor=ACCENT, spaceBefore=12, spaceAfter=4),
        "h2": ParagraphStyle("H2", parent=ss["Heading2"], fontSize=11.5, textColor=DARK, spaceBefore=8, spaceAfter=2),
        "body": ParagraphStyle("B", parent=ss["BodyText"], fontSize=9.5, leading=13, textColor=colors.HexColor("#1e293b")),
        "small": ParagraphStyle("S", parent=ss["BodyText"], fontSize=8, leading=11, textColor=MUTED),
        "bullet": ParagraphStyle("BL", parent=ss["BodyText"], fontSize=9.5, leading=13,
                                  textColor=colors.HexColor("#1e293b"), leftIndent=12, bulletIndent=4),
    }
    return styles
...
def _load() -> dict:
    ...
    return {"ev": ev, "err": err, "abl": abl, "mc": mc, "recs": recs,
            "states": states, "poss": poss, "matches": matches, "dec": dec}
...
def build_technical_pdf() -> Path:
    ...
    st = _styles()
    story = [Paragraph("Adaptive Game Model - Technical Report", st["title"]),
             Paragraph("Event-based tactical decision intelligence: methodology, validation, and reproducibility", st["small"]),
             Spacer(1, 6)]
    ...
    story.append(Paragraph(
        ...
        "augmentation, with match-clustered bootstrap confidence intervals.", st["body"]))

    story.append(Paragraph("2. Data & preprocessing", st["h1"]))
    story.append(Paragraph(f"Source: {mc.get('data_source', {}).get('data_source', 'n/a')} | "
                           f"{ev.get('n_matches')} matches, {ev.get('n_possessions')} possessions.", st["body"]))
    ...
    story.append(Paragraph(
        ...
        "matches. No action from a match appears in more than one split (leakage check passes).", st["body"]))
    ...
    story.append(Paragraph(
        ...
        "is deterministic for offline CI.", st["body"]))
...
def build_coach_pdf() -> Path:
    ...
    d = _load()
    st = _styles()
    story = [Paragraph("Adaptive Game Model - Coach Report", st["title"]),
    ...
    if not d["recs"].empty:
        rows = [["State", "Adjustment", "Uplift / possession", "95% CI", "Conf."]] + [
            [STATE_LABELS.get(r["tactical_state"], r["tactical_state"]),
             ADJUSTMENT_LABELS.get(r["strategy"], r["strategy"]),
             f"{r['aipw_ate']:+.3f}", f"[{r['ci_low']:+.3f}, {r['ci_high']:+.3f}]", r["confidence"]]
        ...
        for _, r in d["recs"].head(6).iterrows():
            ...
            story.append(Paragraph(
                ...
                f"({int(r['n_treated'])} treated / {int(r['n_control'])} control).", st["body"]))
    ...
    for _, r in sample.iterrows():
        story.append(Paragraph(
            ...
            f"outcome level {int(r['outcome_level'])} (shot xG {r['max_shot_xg']:.2f})", st["bullet"]))
...
def build_recruitment_pdf() -> Path:
    ...
    story = [Paragraph("Adaptive Game Model - One-Page Summary", st["title"]),
    ...
    if top is not None and len(top):
        for i, (_, r) in enumerate(top.iterrows(), 1):
            story.append(Paragraph(
                ...
                f"(CI [{r['ci_low']:+.3f}, {r['ci_high']:+.3f}]).", st["bullet"]))
    else:
        story.append(Paragraph("No adjustment estimates reached the sample threshold yet.", st["body"]))
    story.append(Paragraph(
        f"The value model beats every baseline out-of-time (RMSE {ev.get('metrics', {}).get('xgboost_calibrated', {}).get('rmse', 0):.3f} "
        f"vs league-average {ev.get('metrics', {}).get('league_average', {}).get('rmse', 0):.3f}).", st["bullet"]))
    story.append(Paragraph(
        "The system names its tactical states in coaching language and explains its failures - it is built to be "
        "questioned, not to be followed blindly.", st["bullet"]))
    ...
    if top is not None and len(top):
        ...
        story.append(Paragraph(
            ...
            f"{r['aipw_ate'] * 60:.1f} EPV across 60 possessions, at {r['confidence']} confidence.", st["body"]))
    ...
    story.append(Paragraph(
        ...
        "effects (space, timing) are not captured.", st["body"]))
    story.append(Spacer(1, 8))
    story.append(Paragraph("Contact: you@example.com | github.com/your-handle/adaptive-game-model", st["small"]))
...
def build_opponent_pdf(match_id: int | None = None) -> Path:
    ...
    story = [Paragraph("Example Opponent Report", st["title"]),
             Paragraph(
                 f"{info.get('home_team', '?')} vs {info.get('away_team', '?')} "
                 f"- {info.get('match_date', '?')}", st["small"]),
             Spacer(1, 8)]
    ...
    for s in info.get("states", []):
        story.append(Paragraph(
            ...
            f"({s.get('share', 0) * 100:.0f}%)", st["bullet"]))
    ...
    if rec:
        ...
        story.append(Paragraph("Insufficient comparable sequences - more data required.", st["body"]))
