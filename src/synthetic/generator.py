...
TEAMS = [
    ("CF Real Blanco", 1750, 433),
    ("Atletico Verde", 1730, 442),
    ("FC Azul Marino", 1710, 433),
    ("Sevilla Roja", 1690, 4231),
    ("Valencia Naranja", 1660, 433),
    ("Betis Verdiblanco", 1630, 442),
    ("Villarreal Amarillo", 1600, 352),
    ("Sociedad Txuri", 1570, 433),
    ("Celta Celeste", 1540, 4231),
    ("Getafe Azulon", 1510, 442),
]
...
def simulate_match(
    match_id: int, home_id: int, away_id: int, rng: np.random.Generator
) -> list[dict]:
    ...
    for _ in range(n_possessions):
        ...
        play_pattern = (
            "from_kick_off"
            if recovery == "kick_off"
            else ("from_goal_kick" if recovery == "goal_kick" else "regular_play")
        )
        ...
        events.append(
            _event(
                ...
                None,
                False,
                counterpress=counterattack and press > 0,
                rng=rng,
                team_id=attacking_side,
            )
        )
        ...
        for _ in range(n_actions):
            ...
            # advance the ball
            length = float(
                rng.lognormal(1.95, 0.5) if rng.random() < 0.75 else rng.lognormal(2.7, 0.35)
            )
            ...
            if end_x >= 72 and rng.random() < 0.22 + 0.10 * (end_x - 72) / 48:
                ...
                outcome = (
                    ...
                    else SHOT_OUTCOMES[int(rng.integers(1, len(SHOT_OUTCOMES)))]
                )
                if outcome == "Goal":
                    goal = True
            ...
            events.append(
                _event(
                    ...
                    rng=rng,
                    team_id=attacking_side,
                    recipient=attacking_side * 100 + 9,
                )
            )
            x, y = end_x, end_y
